...
| 页面 | 解决什么问题 |
| --- | --- |
| **首页** | 当前窗口、镜子、快速录音、随笔记、常用指令、自动识别音乐播放器和番茄钟集中在一个 Bento 工作台 |
...
| **设置** | 当菜单栏图标被刘海遮挡时，仍可在面板内配置桌面派蒙、API、镜子、首页组件、功能显示、快捷键、数据目录与开机启动 |

派蒙不是第八个孤立页面，而是贯穿折叠刘海、展开工作台和桌面三种状态的统一入口。

「设置 → 这台 Mac → 启动桌面派蒙」可随时关闭或重新开启派蒙本体。关闭后，刘海悬停、桌面宠物和派蒙对话停止运行，但工作台、计时、音乐、待办等功能继续可用；选择会保存在本机，并在下次启动时恢复。派蒙停靠或拖出的位置不会因关闭而被重置。
...
音乐卡片默认自动识别本机已支持且正在运行的播放器，优先选择正在播放的应用；当前支持 Apple Music 与汽水音乐。也可以在「设置 → 音乐播放器」中固定选择。Apple Music 直接同步歌曲名、歌手和播放状态，并使用系统自动化接口完成播放、暂停与切歌；汽水音乐保留本机快捷键适配。
