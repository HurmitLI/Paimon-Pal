...
本文件记录 Paimon Pal 的重要功能、修复与发布变更。尚未发布的改动统一进入「未发布」，创建新版本标签时再归档到对应版本。

## [未发布]

### 修复

- 开发模式不再允许把 `node_modules` 内的 `Electron.app` 注册为 macOS 登录项，避免下次开机出现 Electron 默认欢迎页；正式安装版仍可正常启用开机启动。

## [Paimon Pal 0.5.1] - 2026-09-04

### 新增

- 设置页新增持久化的「启动桌面派蒙」开关；关闭后不再创建宠物窗口或响应刘海悬停，并同步关闭派蒙对话与语音，工作台其他功能保持可用，再次开启时恢复原停靠状态或桌面位置。
- 音乐卡片支持自动识别 Apple Music 与汽水音乐，也可以在设置页手动固定播放器。

### 优化与修复

- 点击派蒙现在只会在角色身侧打开轻量对话气泡，不再连带展开完整刘海工具栏；点击刘海本体仍进入完整工作台。
- 派蒙拖回刘海归位时会同步关闭身侧对话气泡、停止语音并释放输入焦点，不再留下脱离角色的悬空窗口。
- 统一派蒙静止、点击、聆听和说话动画的可见高度与脚底基线，并校正点击表情帧的主体横向偏移，消除打开对话框时的放大回缩和表情抖动。
- 派蒙轻量对话框支持按 `Esc` 立即关闭，并同步停止语音、释放输入焦点；完整刘海工作台原有的 `Esc` 收起行为保持不变。
- 本地语音改为流式播放并预热首轮推理，修复首句开头音色异常、语音延迟过长与重复添加“旅行者”的问题。
- “当前窗口”改用无需屏幕录制权限的 AppKit 读取方案，避免已授权后仍反复提示授权。
- 修复派蒙对话打开时无法切换刘海工作台，以及对话框优先级异常的问题。
...
[未发布]: https://github.com/HurmitLI/Paimon-Pal/compare/v0.5.1...HEAD
[Paimon Pal 0.5.1]: https://github.com/HurmitLI/Paimon-Pal/compare/v0.5.0...v0.5.1
[Paimon Pal 0.5.0]: https://github.com/HurmitLI/Paimon-Pal/releases/tag/v0.5.0
