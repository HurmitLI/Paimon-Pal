...
当前版本：`0.3.0 (9)`，处于个人课程作业验收阶段，尚未完成 Developer ID 正式签名和苹果公证。
...
- 本地语音：内置用户选定的 P1“清脆反应感”原创幻想伙伴固定参考声纹与离线 TTS，支持回复、工具结果和计时结束播报，可在设置中关闭或立即停止。
...
- 对话、语音识别、模型推理和语音生成都在本机完成；AI 对话实验模式不保存原始录音，语音播放后会删除临时 WAV，不会自动下载或上传内容。
...
- 自包含本地 TTS 已在 App 内完成生成、播放、打断、临时文件清理和计时结束播报验证。
